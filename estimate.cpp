...
void HeadPoseEstimation::my_project_points(std::vector<Point3f> points, head_pose pose, std::vector<Point2f>& projected_points) const
{
    for (Point3f p : points) {
        Point3f tmp;
        tmp.x = pose(0, 0) * p.x + pose(0, 1) * p.y + pose(0, 2) * p.z + pose(0,3);
        tmp.y = pose(1, 0) * p.x + pose(1, 1) * p.y + pose(1, 2) * p.z + pose(1,3);
        tmp.z = pose(2, 0) * p.x + pose(2, 1) * p.y + pose(2, 2) * p.z + pose(2,3);

        tmp.x = tmp.x /tmp.z * focalLength + opticalCenterX;
        tmp.y = tmp.y /tmp.z * focalLength + opticalCenterY;

        projected_points.push_back(Point2f(tmp.x, tmp.y));
    }
}
...
//    }
    ...
    putText(_debug,
            "(" + to_string(int(pose(0, 3) * 100)) + "cm, " + to_string(int(pose(1, 3) * 100)) + "cm, "
